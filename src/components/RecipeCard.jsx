import { Box, Card, CardBody, Flex, Heading, Image, Stack, Text, VStack} from "@chakra-ui/react"
            ...
            <CardBody padding="0" align="center">
                ...
                <Stack m="3" spacing="2" >
                    <Box>
                        ...
                        <Heading size="md" m="1" fontWeight="medium">{recipe.recipe.label}</Heading>
                    </Box>
                    <VStack spacing="1.0">
                        <Box>
                        ...
                        <Box>
                            {recipe.recipe.dietLabels.map((label) => {
                                return (
                                    <Tag key={label} bgColor="green.100">{label}</Tag>
                                );
                            })}
                        </Box>
                        <Flex flexDir="row" gap="1.5">
                            ...
                            {recipe.recipe.dishType.map((label) => {
                                return (
                                    <Text key={label} fontWeight="semibold" textTransform="capitalize">{label}</Text>
                                );
                            })}
                            </Box>
                        </Flex> 
                        <Box>
                            ...
                            {recipe.recipe.cautions.map((label) => {
                                return (
                                    <Tag key={label} bgColor="#FCC3B2">{label}</Tag>
                                );
                            })}
                        </Box>
                    </VStack>          
